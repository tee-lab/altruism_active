	...
	for (int i=0; i<nFish; ++i) {
		...
		if (p->ng == 1){
			...
			if ( p->wA == Cooperate){	// individual is cooperator
				p->fitness = (p->kg-1)*b/(p->ng-1) - c[iblock];
			}
			else {	// individual is defector
				p->fitness = p->kg*b/(p->ng-1);
			}
		}
		p->fitness -= cS[iblock]*(p->Rs*p->Rs);	// additional flocking cost
...
int select_reproduce(int iblock){
	...
	float sum_fit = 0;
	for (int i=0; i<nFish; ++i){
		sum_fit += animals[ix2(i,iblock,nFish)].fitness;
	}
	...
	ranges[0] = 0;
	for (int i=0; i<nFish; ++i){
		ranges[i+1] = ranges[i] + animals[ix2(i,iblock,nFish)].fitness/sum_fit;
	}
	...
	for (int i=0; i<nFish; ++i){
		...
		int parent_Ad    = ix2(reproducing_id, iblock, nFish);
		...
		if (b_baseline) offspring[i].Rs = Rs_base;		// in baseline experiment, set Rs = 0
	...
	for (int i=0; i<nFish; ++i){
		animals[ix2(i,iblock,nFish)] = offspring[i];
	}
...
int advanceGen(){
	...
	cudaMemcpy2D( (void*)&(animals[0].pos),  sizeof(Particle), (void*) pos_dev,  sizeof(float2), sizeof(float2), nFish*nBlocks, cudaMemcpyDeviceToHost);

	// for each block, advance generation
	for (int iblock=0; iblock<nBlocks; ++iblock){

		updateGroups(iblock);		// calc group indices 
		calcFitness(iblock);		// calc fitnesses
		writeState(iblock);			// output data if desired
		
		select_reproduce(iblock);	// init next generation 
		disperse(iblock, rDisp);	// dispersal within rDisp units. Random dispersal if rDisp = -1
	}
