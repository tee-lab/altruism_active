...
// =============================================================================
// 		Random numbers from CURAND Mersenne Twister generator (on host)
// 		ADDED by : JAIDEEP
// 		6 May 2013
// =============================================================================

// --- 1-D ---
inline float runif(curandGenerator_t rgen, float rmin=0, float rmax=1){
	static float * rands = new float;
	curandGenerateUniform(rgen, (float*)rands, 1);
	return rmin + (*rands)*(rmax-rmin);
}

inline float rnorm(curandGenerator_t rgen, float mu=0, float sd=1){
	static float * rands = new float;
...
// --- 2-D ---
inline float2 rnorm2(curandGenerator_t rgen, float mu=0, float sd=1){
	static float * rands = new float[2];
	curandGenerateNormal(rgen, (float*)rands, 2, mu, sd);
	return make_float2(rands[0], rands[1]);
}

inline float2 runif2(curandGenerator_t rgen, float xlim, float ylim){
	float x = runif(rgen, -xlim, xlim);
	float y = runif(rgen, -ylim, ylim);
	return make_float2(x,y);
}

inline float2 runif2(curandGenerator_t rgen, float _norm){
...
inline float2 rnorm2_bounded(curandGenerator_t rgen, float mu, float sd, float rmin, float rmax){
